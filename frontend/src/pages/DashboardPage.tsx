...
export default function DashboardPage({ onNavigate, initialTab }: DashboardPageProps) {
  ...
  const stats = useMemo(() => {
    ...
    const todayResults = labResults.filter((r) => r.date === todayStr);
    const recentLabResults = [...labResults]
      .sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime())
      .slice(0, 6);
    const flaggedResults = [...criticalResults, ...abnormalResults]
      .sort((a, b) => {
        const order = { Critical: 0, Abnormal: 1, Normal: 2 };
        return order[a.status] - order[b.status];
      })
      .slice(0, 6);
    // Patients who have at least one lab result with pending/abnormal status
        ...
        <TabsContent value="overview" className="space-y-6">
          ...
          {isLabDoctor && (
            <div className="space-y-6">
              ...
              {/* ── Flagged Results (Critical + Abnormal) ─────────────────────── */}
              <Card>
                <CardHeader className="flex flex-row items-center justify-between pb-3">
                  <CardTitle className="text-base font-semibold flex items-center gap-2">
                    <AlertTriangle className="w-4 h-4 text-orange-500" />
                    Flagged Results
                    {(stats.criticalCount + stats.abnormalCount) > 0 && (
                      <Badge className="ml-1 bg-orange-100 text-orange-700 border-orange-200 text-[10px] px-1.5">
                        {stats.criticalCount + stats.abnormalCount} flagged
                      </Badge>
                    )}
                  </CardTitle>
                </CardHeader>
                <CardContent className="pt-0 space-y-2">
                  {stats.flaggedResults.length === 0 ? (
                    <div className="text-center py-10 text-muted-foreground">
                      <CheckCircle2 className="w-9 h-9 mx-auto mb-2 opacity-40 text-emerald-500" />
                      <p className="text-sm font-medium text-emerald-700 dark:text-emerald-400">All results within normal range</p>
                      <p className="text-xs mt-0.5">No critical or abnormal results to review</p>
                    </div>
                  ) : (
                    stats.flaggedResults.map((r) => {
                      ...
                      return (
                        <div key={r.id} className="flex items-start gap-3 p-3 rounded-lg border bg-muted/30 hover:bg-muted/50 transition-colors">
                          {/* Status indicator */}
                          <div className={`mt-0.5 w-2 h-2 rounded-full shrink-0 ${r.status === "Critical" ? "bg-red-500" : "bg-orange-400"}`} />
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-xs text-muted-foreground mt-0.5">
                              Result: <span className="font-medium text-foreground">{r.result}{r.unit ? ` ${r.unit}` : ""}</span>
                      ...
                      stats.recentLabResults.map((r) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <div className="flex-1 min-w-0">
                              <p className="text-sm font-medium truncate group-hover:text-primary transition-colors">{r.testName}</p>
                ...
                <Card>
                  ...
                  <CardContent className="pt-0 space-y-4">
                    ...
                    <div className="space-y-2.5">
                      {(
                        [
                          { label: "Normal", count: stats.totalLabResults - stats.criticalCount - stats.abnormalCount, color: "bg-emerald-500", textColor: "text-emerald-700 dark:text-emerald-400" },
                          { label: "Abnormal", count: stats.abnormalCount, color: "bg-orange-400", textColor: "text-orange-700 dark:text-orange-400" },
                          { label: "Critical", count: stats.criticalCount, color: "bg-red-500", textColor: "text-red-700 dark:text-red-400" },
                        ] as const
                        ...
                        <div key={label} className="flex items-center gap-3">
                          ...
                          <div className="w-24 h-1.5 bg-muted rounded-full overflow-hidden">
                            <div
                              ...
                              style={{ width: stats.totalLabResults > 0 ? `${(count / stats.totalLabResults) * 100}%` : "0%" }}
                            />
                          </div>
                        </div>
                      ))}
                    ...
                    <div className="border-t pt-4 space-y-2">
                      <div className="flex justify-between text-sm">
                        <span className="text-muted-foreground">Total results uploaded</span>
                      ...
                      <div className="flex justify-between text-sm">
                        <span className="text-muted-foreground">Results uploaded today</span>
